...
tmpfile <- tempfile()
snp_readBed("/home3/vvenka23/FNC-Lasso/RedoPaperCoLaus/dataHg38/hg38/fixFiles2/merged_QC_8.bed",backingfile = tmpfile)
...
X.just.lasso <- FBM(n, p, init=G[,info_snp$`_NUM_ID_`])
  ...
  write.table(cbind(k,R.fnc.lasso, R.sis.lasso, R.just.lasso, R.ct.prs,
                  ...
                  fnc.t*60, sis.t, just.lasso.t*60, ct.t*60),file="otherOp_par",quote=FALSE,row.names=FALSE,col.names= !file.exists("otherOp_par"),append=T)
