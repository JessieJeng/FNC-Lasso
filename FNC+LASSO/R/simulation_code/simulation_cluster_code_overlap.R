########################################################################################################################
############### Knowledge Transfer with False Negative Control Improves the Accuracy of Polygenic Prediction ###########
...
n_target <- 1000 # training + testing
...
f <- function(k) {
  ...
  locate.overlap <- locate[((1-delta)*n_signal+1):n_signal] # locate of signals exist in both base and target data
  locate.target <- locate[(n_signal+1):((2-delta)*n_signal)] # locate of signals only exist in target data
  
  beta.base <- rnorm(((1-delta)*n_signal), 0, h)
  beta.target <- rnorm(((1-delta)*n_signal), 0, h)
  beta.overlap <- mvrnorm((delta*n_signal), rep(0, 2), sigma.beta)
